...
void create_array_of_struct ((struct onegin_data)* inf_about_text)
{
    ...
    {
        assert (number_of_symbol >= 0 && number_of_symbol <= (*inf_about_text).text_len - 1);
        ...
        {
            *((*inf_about_text).index_array_of_struct + index_array_of_struct) = {(const char*) ptr_on_last_line, (const size_t) (ptr_on_symbol_in_text - ptr_on_last_line + 1)};   //Записывает структуру в массив структур.
                                                                                                                        //+1, тк при разности не учитывается первый символ строки.
           
            ptr_on_last_line += number_of_symbol + 1;  //Записывает указатель на новую строку.
